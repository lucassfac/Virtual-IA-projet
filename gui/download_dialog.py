...
from gui.workers import DownloadWorker, SearchWorker
...
class FileSelectorDialog(QDialog):
    """
    Affiche les fichiers .gguf disponibles dans un repo HuggingFace.
    Retourne le fichier sélectionné via .selected_file
    """
    ...
    def _load_files(self):
        ...
        class FileLoader(QThread):
            ...
            def run(self):
                try:
                    files = get_repo_gguf_files(self.repo_id)
                    self.done.emit(files)
                except Exception as e:
                    self.error.emit(str(e))

        self._loader = FileLoader(self.repo_id)
...
class DownloadProgressDialog(QDialog):
    """
    Progression de téléchargement style Steam :
    ...
    def _build(self):
        ...
        # Titre
        name = self.file_info.get("filename", "")
        title = QLabel(name)
        title.setStyleSheet(
            "font-size:14px;font-weight:600;color:#F2F2F7;background:transparent;"
        )
        ...
        # Barre principale — style Steam (grande, colorée)
        self._bar = QProgressBar()
        self._bar.setRange(0, 1000)  # précision 0.1%
        ...
        self._bar.setStyleSheet(
            ...
            "    stop:0 #1A6FFF, stop:0.5 #0A84FF, stop:1 #30D158);"
            "  border-radius:5px;"
            "}"
        )
        ...
        self._pct_lbl.setStyleSheet(
            "font-size:22px;font-weight:700;color:#F2F2F7;background:transparent;"
        )
        ...
        self._speed_lbl.setStyleSheet(
            "font-size:13px;color:#0A84FF;font-weight:500;background:transparent;"
        )
        ...
        self._eta_lbl.setStyleSheet(
            "font-size:11px;color:#636366;background:transparent;"
        )
        ...
        # Barre de détail (mini barre secondaire animée)
        self._detail_bar = QProgressBar()
        self._detail_bar.setRange(0, 0)  # indéterminé au début
    ...
    def _on_error(self, msg: str):
        ...
        self._pct_lbl.setStyleSheet(
            "font-size:22px;font-weight:700;color:#FF453A;background:transparent;"
        )
